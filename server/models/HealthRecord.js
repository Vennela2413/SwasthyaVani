...
const DISEASES = {
  "Malaria":         { s:["fever","chills","sweating","headache","nausea","vomiting","muscle pain","fatigue","shivering"], sev:"high",     e:"🦟", a:"Visit doctor immediately. Take prescribed antimalarial drugs. Use mosquito nets.", te:"మలేరియా", hi:"मलेरिया" },
...
router.post("/predict", async (req, res) => {
  try {
    ...
    // 2. Save if user is logged in — check token from Authorization header
    ...
    if (token) {
      ...
      console.log("ℹ️  No token — skipping save (user not logged in)");
    }
...
// ── POST /api/health/save — explicit save from frontend ─────────────────────
router.post("/save", auth, async (req, res) => {
  try {
    const { symptoms, language, predictions, matched_symptoms, emergency } = req.body;
    const record = await saveRecord(req.user.id, symptoms, language, { predictions, matched_symptoms, emergency });
...
router.get("/history", auth, async (req, res) => {
  try {
    const records = await HealthRecord
      ...
      .sort({ createdAt: -1 })
      .limit(50);
    console.log(`📋 History: ${records.length} records for user ${req.user.id}`);
...
router.get("/stats", auth, async (req, res) => {
  try {
    ...
    res.json({
      ...
      mild:        records.filter(r => r.predictions[0]?.severity === "mild").length,
      moderate:    records.filter(r => r.predictions[0]?.severity === "moderate").length,
      serious:     records.filter(r => r.predictions[0]?.severity === "high").length,
      emergencies: records.filter(r => r.emergency).length,
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
